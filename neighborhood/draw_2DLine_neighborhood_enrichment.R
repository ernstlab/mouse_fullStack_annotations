...
get_state_type <- function(state_annot_fn){
  state_annot_df <- as.data.frame(read.csv(state_annot_fn, header = T, sep = '\t')) ### HAHAHA this line may need to change depending on what state annotation we are using
  state_annot_df$state <- sapply(as.character(state_annot_df$state), function (x){paste0('S', x)})
  return(state_annot_df)
}

draw_2DLine_neighborhood_enrichment <- function (enrichment_fn, save_fig_fn, state_annot_fn) {
	...
	annot_df <- get_state_type(state_annot_fn)
	print(head(annot_df))
...
tss_fn <- args[1] # output of ChromHMM neighborhoodEnrichment
save_tss_fn <- args[2] # where the figures should be stored
tes_fn <- args[3] # output of ChromHMM neighborhoodEnrichment
save_tes_fn <- args[4] # where the figures should be stored
state_annot_fn <- args[5] # where the state annotation are stored
...
draw_2DLine_neighborhood_enrichment(tss_fn, save_tss_fn, state_annot_fn)
...
draw_2DLine_neighborhood_enrichment(tes_fn, save_tes_fn, state_annot_fn)